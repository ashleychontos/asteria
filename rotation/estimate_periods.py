...
def ensemble_plot_teff(path='rotation.csv', path_save='distributions', min_sample=20, save=True,
                       res_teff=100., res_logg=0.1, log=False, save_dist=True, show=True, verbose=True):
    import os
    ...
    subdir = 'delta_t_%d_K_delta_l_%2f_dex'%(int(res_teff),res_logg)
    if save_dist:
        path_save = os.path.join(path_save, subdir)
